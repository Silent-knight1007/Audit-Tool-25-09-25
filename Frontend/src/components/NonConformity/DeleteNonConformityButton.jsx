export default function DeleteNonConformityButton({ onDelete, selectedIds, userRole }) {
  const isDisabled = selectedIds.length === 0 || (userRole !== 'admin' && userRole !== 'auditor' && userRole !== 'superadmin');
  return (
    <button
      onClick={onDelete}
      title={isDisabled ? 'You do not have permission to delete NonConformity' : ''}
      disabled={selectedIds.length === 0 || (userRole !== 'admin' && userRole !== 'auditor' && userRole !== 'superadmin')}
